...
size_t renderControl_decoder_context_t::decode(void *buf, size_t len, IOStream *stream)
{
	...
	while ((len - pos >= 8) && !unknownOpcode) {   
		...
		case OP_rcGetRendererVersion: {
			...
			*(GLint *)(&tmpBuf[0]) = 			rcGetRendererVersion();
			if (useChecksum) {
		...
		case OP_rcGetEGLVersion: {
			...
			*(EGLint *)(&tmpBuf[0 + size_major + size_minor]) = 			rcGetEGLVersion((EGLint*)(outptr_major.get()), (EGLint*)(outptr_minor.get()));
			outptr_major.flush();
			outptr_minor.flush();
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
		...
		case OP_rcQueryEGLString: {
			...
			*(EGLint *)(&tmpBuf[0 + size_buffer]) = 			rcQueryEGLString(var_name, (void*)(outptr_buffer.get()), var_bufferSize);
			outptr_buffer.flush();
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
		...
		case OP_rcGetGLString: {
			...
			*(EGLint *)(&tmpBuf[0 + size_buffer]) = 			rcGetGLString(var_name, (void*)(outptr_buffer.get()), var_bufferSize);
			outptr_buffer.flush();
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
		...
		case OP_rcGetNumConfigs: {
			...
			*(EGLint *)(&tmpBuf[0 + size_numAttribs]) = 			rcGetNumConfigs((uint32_t*)(outptr_numAttribs.get()));
		...
		case OP_rcGetConfigs: {
			...
			*(EGLint *)(&tmpBuf[0 + size_buffer]) = 			rcGetConfigs(var_bufSize, (GLuint*)(outptr_buffer.get()));
			outptr_buffer.flush();
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
		...
		case OP_rcChooseConfig: {
			...
			*(EGLint *)(&tmpBuf[0 + size_configs]) = 			rcChooseConfig((EGLint*)(inptr_attribs.get()), var_attribs_size, size_configs == 0 ? NULL : (uint32_t*)(outptr_configs.get()), var_configs_size);
		...
		case OP_rcGetFBParam: {
			...
			*(EGLint *)(&tmpBuf[0]) = 			rcGetFBParam(var_param);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcCreateContext: {
			...
			*(uint32_t *)(&tmpBuf[0]) = 			rcCreateContext(var_config, var_share, var_glVersion);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcCreateWindowSurface: {
			...
			*(uint32_t *)(&tmpBuf[0]) = 			rcCreateWindowSurface(var_config, var_width, var_height);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcDestroyWindowSurface: {
			...
			rcDestroyWindowSurface(var_windowSurface);
		...
		case OP_rcCreateColorBuffer: {
			...
			*(uint32_t *)(&tmpBuf[0]) = 			rcCreateColorBuffer(var_width, var_height, var_internalFormat);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcOpenColorBuffer: {
			...
			rcOpenColorBuffer(var_colorbuffer);
		...
		case OP_rcCloseColorBuffer: {
			...
			rcCloseColorBuffer(var_colorbuffer);
		...
		case OP_rcSetWindowColorBuffer: {
			...
			rcSetWindowColorBuffer(var_windowSurface, var_colorBuffer);
		...
		case OP_rcFlushWindowColorBuffer: {
			...
			*(int *)(&tmpBuf[0]) = 			rcFlushWindowColorBuffer(var_windowSurface);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcMakeCurrent: {
			...
			*(EGLint *)(&tmpBuf[0]) = 			rcMakeCurrent(var_context, var_drawSurf, var_readSurf);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcFBSetSwapInterval: {
			...
			rcFBSetSwapInterval(var_interval);
		...
		case OP_rcBindRenderbuffer: {
			...
			rcBindRenderbuffer(var_colorBuffer);
		...
		case OP_rcColorBufferCacheFlush: {
			...
			*(EGLint *)(&tmpBuf[0]) = 			rcColorBufferCacheFlush(var_colorbuffer, var_postCount, var_forRead);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcReadColorBuffer: {
			...
			rcReadColorBuffer(var_colorbuffer, var_x, var_y, var_width, var_height, var_format, var_type, (void*)(outptr_pixels.get()));
			outptr_pixels.flush();
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
		...
		case OP_rcUpdateColorBuffer: {
			...
			*(int *)(&tmpBuf[0]) = 			rcUpdateColorBuffer(var_colorbuffer, var_x, var_y, var_width, var_height, var_format, var_type, (void*)(inptr_pixels.get()));
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcOpenColorBuffer2: {
			...
			*(int *)(&tmpBuf[0]) = 			rcOpenColorBuffer2(var_colorbuffer);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcCreateClientImage: {
			...
			*(uint32_t *)(&tmpBuf[0]) = 			rcCreateClientImage(var_context, var_target, var_buffer);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcDestroyClientImage: {
			...
			*(int *)(&tmpBuf[0]) = 			rcDestroyClientImage(var_image);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcSelectChecksumCalculator: {
			...
			rcSelectChecksumCalculator(var_newProtocol, var_reserved);
		...
		case OP_rcGetDisplayWidth: {
			...
			*(int *)(&tmpBuf[0]) = 			rcGetDisplayWidth(var_displayId);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcGetDisplayHeight: {
			...
			*(int *)(&tmpBuf[0]) = 			rcGetDisplayHeight(var_displayId);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcGetDisplayDpiX: {
			...
			*(int *)(&tmpBuf[0]) = 			rcGetDisplayDpiX(var_displayId);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcGetDisplayDpiY: {
			...
			*(int *)(&tmpBuf[0]) = 			rcGetDisplayDpiY(var_displayId);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcGetDisplayVsyncPeriod: {
			...
			*(int *)(&tmpBuf[0]) = 			rcGetDisplayVsyncPeriod(var_displayId);
			if (useChecksum) {
				ChecksumCalculatorThreadInfo::writeChecksum(&tmpBuf[0], totalTmpSize - checksumSize, &tmpBuf[totalTmpSize - checksumSize], checksumSize);
			}
			stream->flush();
		...
		case OP_rcPostLayer: {
			...
			rcPostLayer((const char*)(inptr_name.get()), var_colorBuffer, var_alpha, var_sourceCropLeft, var_sourceCropTop, var_sourceCropRight, var_sourceCropBottom, var_displayFrameLeft, var_displayFrameTop, var_displayFrameRight, var_displayFrameBottom);
